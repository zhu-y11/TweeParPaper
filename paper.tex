...
\nascomment{removed redundant stuff here, please check}
...
dataset, we believe that normalization is an unnecessarily complicated
step.\yicomment{Again we did normalization!} \nascomment{maybe just
  add ``for acronyms'' to the end of that last sentence?}
...
\yicomment{add final data split}\yjcomment{it's better to use table
  ...
  re-stating our data source.} We obtained 3,550 labelled annotated
tweets, split into train/development/test set with /(639 $+$
...
\begin{table}[t]
	...
	\begin{tabular}{rrcc}
		...
		POS tagging &  \it Precision $F_1$ & 93.7 & 92.1 \\
		UD parsing & \it LAS $F_1$ & 74.1 & 70.3 \\
		\hline
	\end{tabular}
	\caption{Evaluating our pipeline against a state-of-the-art pipeline. \nascomment{what is ``precision $F_1$?}\label{tbl:pipline} }
\end{table}
...
\nascomment{changed ``segmentation'' to ``tokenization'' here, twice}, UD parsing
