...
at-mention as an argument \yicomment{@coldplay seems not to be an
  object, could be some modifier like vocative, or some prepositional
  case (the tweets on/about coldplay)}\yjcomment{i am okey with
  current version} \nascomment{changed ``object'' to ``argument'' - better?}.
...
showed the same in our case \nascomment{added to address an open issue}, so 
...
but their implementation works only with the PTB tagset
\nascomment{check here -- seems surprising to me, hopefully that's
  enough for reviewers}, so we exclude
