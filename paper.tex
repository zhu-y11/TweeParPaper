...
and \textit{clitic clusters}, are tokenized.\nss{explain what these two phenomena are. are these the only words that are split?}
...
on social media data is essential according to \citet{eisenstein:2013:NAACL-HLT}.\nss{``It's arguable that normalization...is essential'' suggests that we think it IS essential. Did you mean ``whether'' instead of ``that''?}
In this paper, we don't include normalization and leave it an open question for further study.\nss{Maybe just say: 
Their lemmas include spelling normalization, whereas our lemmas only normalize casing and inflectional morphology (if that is correct). 
...
our and \citeposs{sanguinetti-17} interpretation of headedness is the same, 
