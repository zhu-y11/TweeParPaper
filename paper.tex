...
a new tweet treebank ({\sc Tweebank v2}), which has 3,550 tweets\yicomment{add tweet \#} and 55,607 tokens with
labeled attachments. 
...
consistent annotation \yicomment{annotations?} is still challenging to deliver even with
...
There is a significant \yicomment{significant?} tradeoff between preservation of original tweet content and respecting
the UD guidelines.
...
is tokenized as {\it it} and {\it s} when {\it s} is a copula)\yicomment{are words like gonna called contractions, or do they have another term?}. But acronyms are not touched
...
(\texttt{SYM}), following UD\_English, seen in Figure \ref{fig:non-syn-toks} as an example.
...
forms? \yicomment{this sentence should be deleted or moved to tokenization?} \yicomment{As we already know from previous sections, \emph{idc} is not tokenized and tagged as verb, we follow ...} \yicomment{Normalization is discussed in the 2.5.2 Annotation Process. Actually we did do normalization, but just as auxiliary info, didn't use it for any model.}
...
dataset, we believe that normalization is an unnecessarily complicated step.\yicomment{Again we did normalization!}
...
at-mention as its object\yicomment{or might be treated as 2 non-syntactic tokens and a punctuation, which should all attach to the predicate of the following retweeted tweet.}.  This solution would lead to an uninformative root
...
conventions: ellipsis, irregular word orders, and paratactic sentences \yicomment{or phrases/components}
...
as a sign that a tweet is a reply to a tweet by the mentioned user\yicomment{not necessary, can be just mentioning somebody. should be careful to say it. We can say this construction include 1. reply;  2. mention sb. Both cases are similar to general vocative proper noun case in standard texts. And we need to exclude at-mentions in Retweet Construction.}.  We
treat these at-mentions as vocative expressions, labeling them as
\emph{proper noun} (\texttt{PROPN})\yicomment{here is confusing that PROPN could be thought as a label in dependencies.} and attaching them to the
...
\yicomment{add final data split} We obtained 3,550 labelled annotated tweets, split into train/dev/test set with /(639 $+$ 1000)/(210 $+$ 500)/(201 $+$ 1000) tweets, where each set consists of both old tweets from \citet{gimpel-EtAl:2011:ACL-HLT2011} and new tweets from the Twitter stream. \yicomment{should also mention main annotators (authors) are non-native English speakers?}
...
We trained our tokenizer on the training portion\yicomment{set?} of
...
with our tokenizer. \yicomment{our POS tagger results on automatically tokenized data?}
...
\paragraph{Ensemble.}  Due both\yicomment{?} to ambiguity in the training
...
randomly sampling transitions from the ensemble, \yicomment{we?} found useful
...
Stanford CoreNLP tokenizer\yicomment{why not compare with UD pipe?},
...
importance of segmentation\yicomment{you mean word tokenization or sentence segmentation?}:  without gold segmentation, UD parsing
...
the {\sc Tweebank v2}, which has 3,550 tweets\yicomment{add tweet \#} and 55,607 tokens. We characterize the disagreements
