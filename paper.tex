...
parser achieves an improvement of 2.3 in LAS over \nascomment{which
  comparison is this?} the baseline
...
than 2.3 points \nascomment{again, not sure what this is quoting} in LAS over the baseline parser can be achieved with our
distillation method and that it outperforms other state-of-the-art parsers.
...
hashtags, at-mentions, emoticons,  emojis, and unicode glyphs. 
...
They therefore take a more aggressive strategy, including splitting
clitics from contractions or multiword expressions (e.g., {\it
  gonna}), and sometimes combining several orthographic tokens into a
single syntactic word. \yicomment{does UD support normalization for
...
In our manual tokenization, we do not modify any non-informal \nascomment{is this defined?} token in tweets and leave them as in the original tweets. 
Then we mainly follow UD tokenization. We tokenize contractions and
multiword expressions into their syntactic components, but do not
combine tokens that appear as split in the original tweet.
For example, words like {\it gonna} or {\it can't} will be tokenized
to {\it gon na} and {\it ca n't}, and sequences like {\it Y O} and
{\it R E T W E E T} will be not combined back into single
words.\footnote{Such tokens only account for 0.067\%; we use the UD
  {\it goeswith} relation to resolve these cases in the syntax.}
...
additional 210 tweets sampled from the POS-tagged dataset of \newcite{gimpel-EtAl:2011:ACL-HLT2011} and 2,500 tweets sampled from the Twitter stream from February 2016 to July 2016.\footnote{Data downloaded from \url{https://archive.org/}.}
...
%		\newcite{dyer-EtAl:2015:ACL-IJCNLP} & 77.9 & 73.0 \\
		...
		Distillation ($\alpha =1.0$) & 82.2 & 78.0 \\
		Distillation ($\alpha =0.9$) & 82.4 & 78.1 \\		
		Distillation with exploration & 82.5 & 78.4 \\
		\hline
	\end{tabular}
...
\begin{align}
\min_q \quad  & \alpha \sum_i \underbrace{\sum_{a} q(a \mid s_i) \cdot p(a
...
Our parser achieves an improvement of 2.3  \nascomment{which
  comparison are you quoting here?} in LAS over a strong baseline
