...
baseline parser can be achieved with our distillation method and that it outperforms
...
which is the largest Twitter dependencies annotation
(\S\ref{sec:tweebank}).
...
carried out mostly by nonnative speakers, in a very short amount of
...
There is a significant tradeoff  between preservation of original tweet content and respecting
the UD guidelines.
...
in order to better serve dependency annotation, treats each syntactic word as token.
They therefore more aggressively split
clitics from contractions (e.g., {\it  gonna} is tokenized as {\it gon} and {\it na}; {\it its}
is tokenized as {\it it} and {\it s} when {\it s} is a copula). But acronyms is not touched
...
\yicomment{does UD support normalization for
  acronyms? Did not see it from website, need to check. related to
...
Compared to the coarse-grained POS tagging effort of
...
tokenization.  As discussed above, UD calls for more
aggressive tokenization than that used by
\newcite{gimpel-EtAl:2011:ACL-HLT2011}.  In particular, Gimpel et al.~opted out of tokenizing
...
\begin{align}
{\arg\,\min}_q \quad  & \alpha \sum_i \underbrace{\sum_{a} q(a \mid s_i) \cdot p(a
  \mid s_i)}_{\text{distillation loss}} \\
...
our model with a stack of the state-of-the-art systems (named as the {\it SOTA stack}):
