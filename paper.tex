...
%\newcommand{\yjcomment}[1]{}
%\newcommand{\nascomment}[1]{}
%\newcommand{\yicomment}[1]{}
...
special constructions in tweets. Using the extended guidelines \yicomment{including tokenization, POS tagging and parsing? (need to mention the annotation process/pipeline system not only consists parsing in abstract)}, we create
...
parser achieves an improvement of 2.6 in LAS over \nascomment{which
  comparison is this?}\yjcomment{my bad, it was fixed.} the baseline
and outperforms parsers that are state-of-the-art on other treebanks in both accuracy and speed.
\end{abstract}

\section{Introduction}
NLP for social media messages \yicomment{texts?} is challenging, both \yicomment{not only} because domain
adaptation is required, but also because \yicomment{delete `because' ?} creating annotated datasets
(e.g., treebanks)
...
Stanford dependencies were converted afterwards in their work  \nascomment{by who?}\yjcomment{by themselves}. 
...
and languages \cite{guo-EtAl:2015:ACL-IJCNLP2,TACL892}. We propose \yicomment{believe?} that analyzing
...
tokenization, part-of-speech (POS) tags, and (labeled) \yicomment{no need for parenthesis} universal dependencies.
We characterize the disagreement among our annotations \yicomment{annotation(s) and disagreement(s). check abstract, same sentence, different singularity/plurality. I think should be `disagreement' and `annotations'} and find that
consistent annotation \yicomment{again, it needs to be the same all the time at least for me.} is still challenging to deliver even with
...
bidirectional LSTM (bi-LSTM)\yicomment{need cite lstm/bi-lstm here?} tokenizer, a word cluster-enhanced POS
tagger \citep[following][]{owoputi-EtAl:2013:NAACL-HLT} \yicomment{no need for parenthesis?}, and a stack LSTM parser
...
than 2.6 points \nascomment{again, not sure what this is quoting}\yjcomment{fixed} in LAS over the
baseline parser can be achieved with our distillation method and that \yicomment{delete `that'?} it outperforms
other state-of-the-art parsers in both accuracy and running speed.
...
tokenization, part-of-speech \yicomment{POS} tagging, and labeled universal dependencies annotation.
We also characterize the difficulties of creating such annotation.

\item We introduce and evaluate a \yicomment{pipeline} system to parse the raw tweet text into
universal dependencies (\S\ref{sec:parsing}).  Experimental results show
it performs better than a stack of the state-of-the-art alternatives.
\nascomment{say it
  performs better than existing systems?}\yjcomment{added.}
...
We release our dataset and our \yicomment{delete `our'} system as open-source software \yicomment{delete `as open-source software'?} at
\url{http://anonymized}. 
...
which is \yicomment{was} the largest \yicomment{unlabelled} Twitter dependencies annotation
(\S\ref{sec:tweebank}).
...
The annotation effort we describe stands in contrast to \yicomment{the?} previous work
...
929 tweets.  Their annotations added \yicomment{unlabelled} dependencies to a portion of the data
...
carried out mostly by nonnative speakers \yicomment{should not emphasize non-native speakers (v2 also built by non-native speakers), but emphasize annotators with only cursory training, no clear annotation guidelines, no consensus/agreement on controversial cases, allow annotators to underspecify part of the tree and allow multiple different trees for the same tweets}, in a very short amount of
...
\item Allowing an annotator to exclude tokens from the dependency
  ...
  tokens were excluded because they were deemed ``non-syntactic''\yicomment{(e.g. retweet sign, topical hashtags, ...)}.
...
There is a significant \yicomment{significant? not really} tradeoff between preservation of original tweet content and respecting
the UD guidelines.
...
is tokenized as {\it it} and {\it s} when {\it s} is a copula)\yicomment{I think the examples are not contractions (can't), is there a name for such words?}. But acronyms are not touched
in the UD tokenization guidelines. Thus, we follow the UD tokenization for contractions
and left acronyms like {\em idc} as a single token. \nascomment{unless we make a change, I think we need to say
...
In the different direction of splitting tokens, UD guidelines also suggest to merge \yicomment{suggest merging?}
...
(e.g. {\it Y O}, and {\it R E T W E E T}) but didn't combine them due to simplification.
...
Before turning to UD \yicomment{dependencies} annotations, we (re)annotated the data with 
POS \yicomment{tags}, for consistency with other UD efforts,
...
\newcite{owoputi-EtAl:2013:NAACL-HLT}.  \yicomment{The conflicts should refer to a {\bf noncorresponding tag} from that of gimpel to UDEnglish, cuz they dont have the same tagset.} In these cases, we always
...
Compared to the coarse-grained \yicomment{fine-grained?}POS tagging effort of
...
\newcite{gimpel-EtAl:2011:ACL-HLT2011} \yicomment{YICOMMENT: should be OCONNOR's paper? TOKENIZATION?}. In particular, Gimpel et al.~\yicomment{SAME HERE} opted out of tokenizing
...
forms? \yicomment{this sentence should be deleted} \yicomment{As we already know from previous sections, \emph{idc} is not tokenized and tagged as verb, we follow ...}
We follow \citet{kong-EtAl:2014:EMNLP2014} and annotate their
...
dataset, we believe that normalization is an unnecessarily complicated step.\yicomment{this should be moved to tokenization section, but we can mention again here for emphasizing.}
...
In our annotation, there are five \yicomment{six} types of non-syntactic tokens: 
sentiment emoticons, retweet markers, topical hashtags, referential URLs, and
truncated words \yicomment{and retweet at-mentions}.\footnote{The tweets we analyze have at most 140
...
7.5\% \yicomment{should change according to tab1} of all tokens are non-syntactic; detailed statistics can be found in Table \ref{tbl:non-synt-prop}.
...
used as content words within a sentence\yicomment{at-mentions can be a normal vocative proper noun}; see Figure
\ref{fig:informal-toks}.  Therefore, the criteria for \yicomment{whether} annotating a
...
conventions:\yicomment{shall we mention how we segment sentences? Seems relevant here}
\begin{itemize}
...
In order to keep our annotation conventions in line with UD norms \yicomment{and preserve the original tweets as much as possible},
...
we also gave a clear\yicomment{much clearer} definition of non-syntactic tokens, which helped
...
and attaching \emph{RT} to the predicate of the following sentence\yicomment{since we have introduced retweet at-mentions in the non-syntactic tokens, here we can just briefly justify our choice.}. 
...
conventions: ellipsis, irregular word orders, and paratactic sentences \yicomment{or phrases/component}
...
as a sign that a tweet is a reply to a tweet by the mentioned user\yicomment{not necessary, can be just mentioning somebody. should be careful to say it. We can say this construction include 1. reply;  2. mention sb. Both cases are similar to general vocative proper noun case in standard texts}.  We
treat these at-mentions as vocative expressions, labeling them as
\emph{proper noun} (\texttt{PROPN})\yicomment{here is confusing that PROPN could be a label in dependencies. We can say in POS sections that the decisions on some twitter specific tokens can be made only analyzing syntax together.} and attaching them to the
following predicate with the label \emph{vocative} (see Figure \ref{fig:informal-toks} for an example) \yicomment{just as UD guidelines}.  
...
Following the guidelines presented above, we create a new annotated\yicomment{del?} Twitter \yicomment{Twitter dependency} treebank, which we call {\sc Tweebank v2}.
...
Before annotating, they were given a tutorial overview of the \yicomment{general} UD
annotation conventions and our guidelines \yicomment{specifically for annotating tweets}.
...
parser were trained on the annotated data from the first stage (1,050\yicomment{changed to 1050} tweets in total),
and \yicomment{were?}used to automatically analyze the sampled 2,500 tweets.  Authors 
of this paper manually corrected the parsed data. Finally, an extra
layer of word-level normalization\yicomment{see 10 th col of conllu data, also mention we could recover the original tweets also from this col (tokenization)} was manually annotated for data
analysis purposes.\yicomment{should mention tokenization process, say we tokenize V1 and train tokenizer for V2 data, then manually correct them. Good story :)}
...
in the second stage\yicomment{tokenization is easier, did not do inter-annotator agreement}. The agreement on POS is 96.6\%, the unlabeled
...
involve entity names (30.6\%) \yicomment{noun phrases or proper noun phrases, do we need example here?} and topical hashtags (18.1\%).
Taking the example in Figure \ref{fig:non-syn-toks}, ``Fix you'' \yicomment{need to relate to previous error analysis, entity names maybe?}can
be understood as a verbal phrase but also as the name of the Coldplay's
single and tagged as proper noun. \yicomment{need to say the reason why dependency inter-annotator agreement is low? 1. POS disagreement lead to big difference in understanding the semantics. 2. Some special constructions like ellipsis lead to different view of tweets. Fig3 is an example of 2.}An example of a disagreement on
...
which has stronger representation power. \nascomment{we need to say what their model was;
  why does ours work better?  from their paper, it seems that they
  used a GRU instead of an LSTM ... is that all?  do we win because of
better tuning, or what?}\yjcomment{done.}
...
of data, making speed an important desideratum. We therefore 
...
All the experiments were ran on a Xeon E5-2670 2.6 GHz machine.
...
each of our models and the compared ones. \nascomment{please check! we did this for all systems,
  not just ours, right?}\yjcomment{yes. added}

\begin{table}[t]
	\centering
	\begin{tabular}{rccc}
		\hline
		\it System & UAS & LAS & SP \\
		\hline
...
%		\newcite{dyer-EtAl:2015:ACL-IJCNLP} & 77.9 & 73.0 \\
	...
	\caption{Dependency parser comparison on \textsc{Tweebank v2},
           with automatic POS tags. The {\it SP} column shows
           the parsing speed evaluated by the number of thousand tokens
           the model processed per second. For fair comparison, we limit the
           number of CPU used in \citet{DBLP:journals/corr/DozatM16} experiments to 1.
...
in the CoNLL 2017 evaluations, due to
 ...
 parsers requiring superlinear runtime. \nascomment{can we quantify
   how much time each one takes, either in tweets parsed per second or
   seconds per tweet?}\yjcomment{done.}
Both of the comparison systems are re-trained on the same data as our
system.
Our system lags behind by nearly two LAS
...
\begin{table*}[t]
	...
	\begin{tabular}{rrcc}
		\hline
		\it Pipeline & Metrics & ours & SOTA stack\\
		\hline
		Tokenization & \it F1 & 98.3 & 96.6 \\		
		POS tagging &  \it Precision F1 & 93.7 & 92.1 \\
		Universal dependencies & \it LAS F1 & 74.1 & 70.3 \\
		\hline
	\end{tabular}
	\caption{Evaluating our model in pipeline
		manner. \nascomment{should this be ``F1''?  shouldn't the
			last line be LAS?}\yjcomment{added metrics column}\label{tbl:pipline} }
\end{table*}

\paragraph{Ensemble.}  Due both to ambiguity in the training
...
members) \nascomment{check} \yjcomment{checked}.
...
our model with a stack of the state-of-the-art systems (namely, the {\it SOTA stack}):
Stanford CoreNLP tokenizer,
\citet{owoputi-EtAl:2013:NAACL-HLT}'s tagger, and \citet{DBLP:journals/corr/DozatM16}'s parser.
Our system differs with the SOTA stack in the tokenization and parser components.
From Table \ref{tbl:pipline}, our system outperforms the SOTA stack when
evaluated in pipeline manner.
The results also emphasize the
importance of segmentation:  without gold segmentation, UD parsing
...
among our annotations and argue that inherent ambiguity in this genre
makes
consistent annotaiton a challenge.  Using this new treebank,
...
Our parser achieves an improvement of 2.6  \nascomment{which
  comparison are you quoting here?}\yjcomment{fixed} in LAS over a strong baseline
