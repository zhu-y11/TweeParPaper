...
\newcommand{\yicomment}[1]{\textcolor{red}{[#1 ---\textsc{nas}]}}
...
a new tweet treebank ({\sc Tweebank v2}), which has 55,607 tokens with labelled attachements. 
It is more than 4 times larger than the unlabelled {\sc Tweebank v1} \citep{kong-EtAl:2014:EMNLP2014}. We characterize the disagreements
...
method in training the parser in our pipeline which achieve 2.3 points improvement
without sacrificing efficiency and outperform other state-of-the-art parsers
in our comparison.
\end{itemize}
...
Then we introduce the difference in our part-of-speech (re)annotation with 
\citet{gimpel-EtAl:2011:ACL-HLT2011} which {\sc Tweebank v1} was built 
on (\S\ref{sec:pos-anno}). Then, we move to our efford of adopting the
UD convention to cover tweets specific constructions. At last, we describ
our process of creating new tweet treebank and characterize
...
{\sc Tweebank v2} is built on the original data of {\sc Tweebank v1} (Kong et al., 2014, 840 unique tweets, including training and test set), along with additional 201 tweets sampled from \newcite{gimpel-EtAl:2011:ACL-HLT2011} and 2,500 tweets sampled from the Twitter stream from February 2016 to July 2016.
...
\begin{figure}[t]
	...
	\caption{The bi-LSTM postagger model that tags `it s gon na be' as {\tt PRON AUX VERB PART AUX}.}\label{fig:pos-model}
\end{figure}
